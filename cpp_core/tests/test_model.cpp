...
void test_forward_pass() {
    ...
    assert(output.decision.shape[1] == cfg.output_dim);
...
void test_forward_with_manifold() {
    ...
    assert(output.manifold.shape[0] == 1);
    assert(output.manifold.shape[1] == 32);
    
    std::cout << "  Manifold shape: [" << output.manifold.shape[0] << ", " << output.manifold.shape[1] << "]" << std::endl;
...
void test_memory_module() {
    ...
    assert(result.retrieved.shape[1] == 64);
    assert(result.attention.shape[1] == 16);
