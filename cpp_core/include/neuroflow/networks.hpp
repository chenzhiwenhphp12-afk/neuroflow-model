...
public:
    ...
    Linear(size_t in_features, size_t out_features, bool use_bias = true, bool quant = false)
        ...
        if (quant) {
            ...
            for (size_t i = 0; i < n; ++i) {
                w[i] = (static_cast<float>(std::rand()) / RAND_MAX - 0.5f) * 2 * scale;
            }
        }
        
        if (use_bias) {
            bias = Tensor({out_features}, QuantType::FP32);
            memset(bias.data.get(), 0, bias.data_size);
        }
    }
    
    Tensor forward(const Tensor& input) {
        Tensor output({input.shape[0], weight.shape[0]}, QuantType::FP32);
    ...
    void quantize() {
        ...
        Tensor new_weight({weight.shape[0], weight.shape[1]}, QuantType::INT8);
        Tensor scale({weight.shape[0]}, QuantType::FP32);
...
class Sequential {
public:
    std::vector<std::shared_ptr<void>> layers;
    ...
    std::shared_ptr<T> get(size_t idx) {
        return std::static_pointer_cast<T>(layers[idx]);
    }
};
...
public:
    ...
    Output forward(const Tensor& x, const Tensor* baseline = nullptr) {
        ...
        if (baseline) {
            Tensor diff = x.clone();
            TensorOps::add(diff, *baseline);  // x - baseline (需要修改)
            ...
            out.anomaly = Tensor({x.shape[0], 1}, QuantType::FP32);
        }
