...
public:
    ...
    Tensor forward(const Tensor& x, bool use_cache = true) {
        ...
        size_t seq_len = x.shape.size() > 1 ? x.shape[1] : 1;
        size_t input_dim = x.shape.size() > 2 ? x.shape[2] : (x.shape.size() > 1 ? x.shape[1] : d_model);
        
        // 确定实际维度
        if (x.shape.size() == 2 && x.shape[1] == d_model) {
    ...
    void clear_cache() {
        cache_len = 0;
        memset(cache.data.get(), 0, cache.data_size);
    }
...
public:
    ...
    MemoryConsolidationModule(size_t input_dim, size_t slots = 64, size_t dim = 128, float ltp = 0.01f)
        ...
        for (size_t i = 0; i < memory_bank.numel(); ++i) {
            m[i] = (static_cast<float>(std::rand()) / RAND_MAX - 0.5f) * 0.04f;
        }
...
public:
    ...
    void save_to_disk(size_t id) {
        ...
        f.write(reinterpret_cast<char*>(data), page.data.data_size);
        f.close();
    }
    ...
    void load_from_disk(size_t id) {
        ...
        float* data = page.data.as_fp32();
        f.read(reinterpret_cast<char*>(data), page.data.data_size);
        f.close();
        page.in_memory = true;
        page_order.push(id);
    }
    ...
    Stats get_stats() {
        ...
        for (auto& [id, page] : pages) {
            if (page.in_memory) {
                s.in_memory_pages++;
                s.total_memory_bytes += page.data.data_size;
            } else {
                s.on_disk_pages++;
            }
        }
